...
const LINKS = [
  { label: "Wedding Stories", href: "/stories" },
  { label: "Wedding Films", href: "/films" },
  { label: "Couple Shoot", href: "/couple-shoot" },
          ...
          <nav className="hidden items-center gap-5 lg:flex xl:gap-7">
            {LINKS.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="whitespace-nowrap text-[12px] text-ink/80 transition-colors hover:text-ink"
              >
                {l.label}
              </Link>
            ))}
            <Link
              href="/contact"
              className="whitespace-nowrap rounded-none border border-ink px-4 py-2 text-[12px] text-ink transition-colors hover:bg-ink hover:text-paper"
            >
              Let&rsquo;s Talk
            </Link>
          </nav>
